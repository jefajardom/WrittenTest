...
app.config['MYSQL_DB'] = 'WrittenTest'
...
def get_all_machines():
    ...
    cur.execute("SELECT * FROM WrittenTest.tasks")
    rv = cur.fetchall()
    return jsonify(rv)
...
def get_all_job():
    cur = mysql.connection.cursor()
    cur.execute("SELECT * FROM WrittenTest.job where id = "+id)
    rv = cur.fetchall()
    return jsonify(rv)
...
def update_job(id):
    ...
    cur.execute("UPDATE WrittenTest.Job SET name = '" + str(title) + "' where id = " + id)
